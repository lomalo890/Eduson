...
cars = ['Honda', 'BMV']
record = zip(id, names, cars) #зависит от самого короткого, но посмотрите на строку 60 и импортируте from itertools import zip_longest

print(list(record)) #[(1, 'Антон', 'Honda'), (2, 'Ann', 'BMV')]

record = zip_longest(id, names, cars)
print(list(record)) #[(1, 'Антон', 'Honda'), (2, 'Ann', 'BMV'), (3, 'John', None), (4, None, None), (5, None, None)]

record = zip_longest(id, names, cars, fillvalue="Нет")
print(list(record)) # [(1, 'Антон', 'Honda'), (2, 'Ann', 'BMV'), (3, 'John', 'Нет'), (4, 'Нет', 'Нет'), (5, 'Нет', 'Нет')]
...
print(leader_dict) # {1: 'Антон', 2: 'Ann', 3: 'John', 4: 'Joseph', 5: 'Lisa', 6: 'George', 7: 'Lesya'}
